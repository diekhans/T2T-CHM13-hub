...
# hand errors:
Expecting at least 3 words line 852031276 of A.sort.tmp.bed got 2
Expecting at least 3 words line 130294431 of B.sort.tmp.bed got 2
A.bad:chr7	52
B.bad:chr10	992698
# so remove with awk

# still get overlap errors, so remove with bedGraphLegalize 
Error - overlapping regions in bedGraph line 28 of A.sort.tmp.bed
Error - overlapping regions in bedGraph line 9 of B.sort.tmp.bed

zcat AllChrs_BOTH_chm13-5A_proseq_cutadapt-2_bt2-vs-filtered_bt2-N0-k100-chm13-20200922_sort_INT_chm13-PCRfree_asm-20200922_21mer-single.bedgraph.gz | csort -k 1,1 -k 2,2n | tawk 'NF==4' | bedGraphLegalize -report=A.error.lst stdin stdout | csort -k 1,1 -k 2,2n >A.sort.tmp.bed &
zcat AllChrs_BOTH_chm13-5B_proseq_cutadapt-2_bt2-vs-filtered_bt2-N0-k100-chm13-20200922_sort_INT_chm13-PCRfree_asm-20200922_21mer-single.bedgraph.gz | csort -k 1,1 -k 2,2n | tawk 'NF==4' | bedGraphLegalize -report=B.error.lst stdin stdout | csort -k 1,1 -k 2,2n >B.sort.tmp.bed &

bedGraphToBigWig A.sort.tmp.bed  ../genome/t2t-chm13-v1.0.sizes proSeqMerylA.bigWig >&A.log &
bedGraphToBigWig B.sort.tmp.bed  ../genome/t2t-chm13-v1.0.sizes proSeqMerylB.bigWig >&B.log &
...
either. These tracks were mostly for looking in centromeric/telomeric regions
as they are intersected with unique 21-mers. I’m working on generating them as
separate strands, but it’s a little more intensive, so just wanted to get
these two tracks up there in the interim (as long as it’s not too much
trouble!).

I’m not sure why these overlaps exist. The files were just made by
using bedtools intersect to intersect the PRO-seq reads with the unique
k-Mers.

## debug overlapping regions
Savannah Hoyt  4:18 PM
I uploaded the three files involved in the intersections to: /team-epigenetics/20200922-v1.0_chm13_PROseq/Bowtie2-aligned_Meryl-Intersections/
The files are: (PRO-seq files) 
   CHM13-5A_proseq_cutadapt-2_bt2-vs-filtered_bt2-N0-k100-chm13-20200922_sort.bedgraph
   CHM13-5B_proseq_cutadapt-2_bt2-vs-filtered_bt2-N0-k100-chm13-20200922_sort_BOTH.bedgraph
   (Meryl unique k-mers) IlluminaPCRfree_20200922-assembly_21mers_single.meryl.bed
-a bedgraph -b kmers

cd tmp
tawk '$1=="chr22"' CHM13-5A_proseq_cutadapt-2_bt2-vs-filtered_bt2-N0-k100-chm13-20200922_sort.bedgraph >A.chr22.bedgraph
 sort  -k 2,2n A.chr22.bedgraph >A.chr22.sort.bedgraph 

bedGraphToBigWig A.chr22.sort.bedgraph ../../genome/t2t-chm13-v1.0.sizes A.chr22.bigwig
# ok!

tawk '$1=="chr22"' IlluminaPCRfree_20200922-assembly_21mers_single.meryl.bed  >meryl.chr22.bed


bedtools intersect -a A.chr22.sort.bedgraph -b meryl.chr22.bed >A.chr22.intersect.bedgraph
sort -k 2,2n A.chr22.intersect.bedgraph > A.chr22.intersect.sort.bedgraph
bedGraphToBigWig A.chr22.intersect.sort.bedgraph ../../genome/t2t-chm13-v1.0.sizes A.chr22.intersect.bigWig
Error - overlapping regions in bedGraph line 2 of A.chr22.intersect.sort.bedgraph

# make tiny example
  tawk '$2 <=1000' A.chr22.sort.bedgraph >A.tiny.bedgraph
  tawk '$2 <=1000' meryl.chr22.bed >meryl.tiny.bed 

bedtools intersect -a A.tiny.bedgraph -b meryl.tiny.bed > A-intersect.tiny.bedgraph
