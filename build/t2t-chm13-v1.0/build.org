...
* gencodeBlat
### NOTE for next time, the RNA FASTAs don't include sequences on unplaced/randoms!!
...
cd crispr
~/compbio/nanopore/projs/t2t-chm13/kent/src/hg/utils/automation/doCrispr.pl -forHub -continue indexFa -stop indexFa -buildDir $(pwd)/build -twoBit $(pwd)/../genome/t2t-chm13-v1.0.2bit t2t-chm13-v1.0 >&log

# code somewhere still wannts:
/hive/data/outside/crisprTrack/crispor/genomes/t2t-chm13-v1.0/t2t-chm13-v1.0.fa
so make 
...
indexFa :-)
ranges :-)
guides - run failed jobs with 16gb, then 32gb, then finish runGuides.bash by remove used of 
specScoreJobList :-)
specScores :-)
...
~/compbio/nanopore/projs/t2t-chm13/kent/src/hg/utils/automation/doCrispr.pl -continue  -stop  -forHub -buildDir $(pwd)/build -twoBit $(pwd)/../genome/t2t-chm13-v1.0.2bit t2t-chm13-v1.0 >&log &
